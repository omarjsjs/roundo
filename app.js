/* Roundo v9 – Back-compat: no optional chaining; Modes → Lobby → Questions + Timer + Score/Streak + Rewards */
(() => {
  // ===== i18n =====
  const STR = {
  ...
  function setLang(l){
    ...
    safeText("t_app", t("app"));
    safeText("t_badge", STR[lang].badge);
    safeText("t_home", t("home"));
    safeText("t_modes", t("modes"));
    safeText("t_lobby", t("lobby"));
    safeText("t_store", t("store"));
    const bl = document.getElementById("btnLang");
    if (bl) bl.textContent = (l==="ar"?"EN":"AR");
    render();
  }
  ...
  async function loadContent(){
    ...
    try{
      ...
      if (!res.ok) throw new Error("content.json missing");
      state.content = await res.json();
    }catch(e){
      // احتياطي بسيط حتى لا يتوقف التطبيق
      state.content = {
  ...
  function wireModes(){
    $$("[data-start-mode]").forEach(function(btn){
  ...
  function renderLobby(){
    return wrapPhone(
      screen(t("lobbyTitle"),
        ...
        + '<div class="card"><div class="h2">'+t("players")+'</div>'
        + '<div class="row" style="gap:8px;flex-wrap:wrap"><span class="badge">'+t("you")+'</span>'
        + Array.from({length:state.lobby.players-1}).map(function(_,i){ return '<span class="badge">Friend#'+(200+i)+'</span>'; }).join("")
        + '</div></div>'
  ...
  function wireLobby(){
    ...
    if (bp) bp.addEventListener("click", function(){
      state.lobby.powerups = !state.lobby.powerups; render();
    });
    const bs = $("#btnStart");
  ...
  function renderQuestion(){
    ...
    const opts = q.answers.map(function(a,i){ return {txt:(lang==="ar"?a.ar:a.en), ok:!!a.correct, i:i}; });
    return wrapPhone(
      screen(t("questionTitle"),
        '<div class="row" style="justify-content:space-between;margin-bottom:8px">'
        + '<span class="kbd">'+t("score")+': '+state.score+'</span>'
        + '<span class="kbd">'+t("streak")+': '+state.streak+'</span></div>'
  ...
  function wireQuestion(){
    ...
    $$(".opt").forEach(function(btn){
      btn.addEventListener("click", function(){
        ...
        $$(".opt").forEach(function(b){ b.disabled = true; });
  ...
  function renderCustomization(){
    ...
    const equippedBadges = Object.keys(state.equipped).map(function(slot){
      const id = state.equipped[slot];
      return id ? '<span class="badge">'+slot+': '+id.replace(/_/g," ")+'</span>' : '';
    ...
    function outfitCard(o){
      ...
      return (
        ...
        + (!isOwned
            ...
            : (isEquipped
                ? '<span class="badge">'+t("owned")+'</span> <button class="btn" data-unequip="'+o.slot+'">'+t("unequip")+'</button>'
    ...
    return wrapPhone(
      screen(t("avatarTitle"),
        '<div class="card avatar" style="width:100%"><div class="preview">'+previewSVG()+'</div>'
        + '<div><div><strong>'+((lang==="ar")?char.name_ar:char.name_en)+' — '+char.default.colorway+'</strong></div>'
        + '<div class="row" style="margin-top:6px">'+equippedBadges+'</div></div></div>'
  ...
  function wireCustomization(){
    $$("[data-buy]").forEach(function(btn){
      btn.addEventListener("click", function(){
        ...
        const item = state.content.cosmetics.find(function(x){return x.id===id;});
    ...
    $$("[data-equip]").forEach(function(btn){
      btn.addEventListener("click", function(){
    ...
    $$("[data-unequip]").forEach(function(btn){
      btn.addEventListener("click", function(){
  ...
  function renderStore(){
    const c = state.content;
    const items = []
      .concat(c.store.daily.map(function(id){return c.cosmetics.find(function(x){return x.id===id;});}))
      .concat(c.store.weekly.map(function(id){return c.cosmetics.find(function(x){return x.id===id;});}))
      .filter(Boolean);

    return wrapPhone(
      screen(t("storeTitle"),
        items.map(function(o){
          return '<div class="card store-item" style="width:100%"><div>'
            + '<div><strong>'+o.id.replace(/_/g," ")+'</strong></div>'
            + '<div class="price">'+fmtPrice(o.price)+'</div>'
            + '</div><a class="btn" href="#/customization">'+t("equip")+'</a></div>';
        }).join("")
      )
    );
  }
  ...
  if (btnL) btnL.addEventListener("click", function(){ setLang(lang==="ar"?"en":"ar"); });
  const btnT = document.getElementById("btnTheme");
  if (btnT) btnT.addEventListener("click", function(){ setTheme(theme==="dark"?"light":"dark"); });
